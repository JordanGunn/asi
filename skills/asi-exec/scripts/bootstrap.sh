...
# asi-exec bootstrap script
# Checks environment prerequisites and (optionally) installs deps with explicit opt-in.

usage() {
  cat <<'EOF'
Usage: scripts/bootstrap.sh [--check] [--print-install] [--install --yes]

Default: --check

Checks:
  - jq available (required for task selection and state updates)
