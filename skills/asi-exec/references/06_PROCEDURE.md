...
1. Confirm PLAN.md path is provided or discoverable
2. Confirm PLAN.md has `status: approved`
3. Confirm TODO.md exists
4. Run drift detection (`validate --check plan-drift`)
5. If drift detected, halt and report

---

## Step 1: Select Task

1. Parse TODO.md task list
...
1. Update task status to `in_progress` in TODO.md
2. Log checkpoint: "Starting task {task_id}"
3. Read PLAN.md section referenced by task's `source_section`
...
2. Update task status to `done` in TODO.md
3. Log checkpoint: "Completed task {task_id}"
4. Produce execution receipt
5. Append receipt to RECEIPT.md

**Output:** Task marked done, receipt appended to RECEIPT.md
