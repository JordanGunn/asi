...
# asi-plan kickoff-phase inject script
...
inject_step_1() {
    ...
    if ! command -v jq &>/dev/null; then
        echo "ERROR: jq required for JSON parsing. Run scripts/kickoff-bootstrap.sh --check for install guidance." >&2
        return 1
    fi
...
if [[ $result -eq 0 ]]; then
    ...
    cat <<EOF
{
  "action": "asi-plan-kickoff-inject",
  "step": $STEP,
  "status": "complete",
  "timestamp": "$timestamp",
  "input_file": "$INPUT"
