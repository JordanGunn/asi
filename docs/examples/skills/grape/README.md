...
1. compiles the prompt into schema-shaped receipts:  
   - scan: `grape_surface_plan_v1`, then  
   - search: `grape_intent_v1` + `grape_compiled_plan_v1`
...
3. Agents enforce the contract by running `scripts/plan.{sh,ps1}`, which:  
   - validates the plan,  
   - echoes the compiled receipt, and  
   - executes exactly the declared arguments.
