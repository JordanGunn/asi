...
If invoked via `/grape <prompt>`, the agent first compiles a `grape_surface_plan_v1` and runs
`scripts/scan.sh`/`scripts/scan.ps1`, then compiles `grape_intent_v1` + `grape_compiled_plan_v1`
before running `scripts/plan.sh`/`scripts/plan.ps1 --stdin` (see `07_COMPILER_CONTRACT.md`),
which enforces the plan and gates `scripts/grep.sh`/`scripts/grep.ps1`.
